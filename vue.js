var Vue = (function (exports) {
    ...
    function reload(id, newComp) {
        ...
        Array.from(record).forEach((instance) => {
            ...
            if (!hmrDirtyComponents.has(comp)) {
                ...
                newComp = isClassComponent(newComp)
                    ? newComp.__vccOpts
                    : newComp;
                extend(comp, newComp);
        ...
        const props = instance.vnode.props || EMPTY_OBJ;
        {
            ...
            if (emitsOptions) {
                if (!(event in emitsOptions)) {
                    if (
                        !propsOptions ||
                        !(`on` + capitalize(event) in propsOptions)
                    ) {
                        warn(
                            `Component emitted event "${event}" but it is neither declared in ` +
                                `the emits option nor as an "on${capitalize(
                                    event
                                )}" prop.`
                        );
                    }
    ...
    function filterSingleRoot(children) {
        ...
        return filtered.length === 1 && isVNode(filtered[0])
            ? filtered[0]
            : null;
    }
  ...
  */
    ...
    function baseCreateRenderer(options, createHydrationFns) {
        ...
        const mountComponent = (
            ...
            if (instance.asyncDep) {
                ...
                if (!initialVNode.el) {
                    const placeholder = (instance.subTree = createVNode(
                        Comment
                    ));
    ...
    function recordInstanceBoundEffect(effect) {
        if (currentInstance) {
            (currentInstance.effects || (currentInstance.effects = [])).push(
                effect
            );
        }
    }
    ...
    function processFor(node, dir, context, processCodegen) {
        ...
        if (!parseResult) {
            context.onError(
                createCompilerError(
                    31 /* X_V_FOR_MALFORMED_EXPRESSION */,
                    dir.loc
                )
            );
            return;
        ...
        return function postTransformElement() {
            ...
            if (node.children.length > 0) {
                if (vnodeTag === KEEP_ALIVE) {
                    ...
                    if (node.children.length > 1) {
                        context.onError(
                            createCompilerError(
                                44 /* X_KEEP_ALIVE_INVALID_CHILDREN */,
                                {
            ...
            if (patchFlag !== 0) {
                ...
                if (dynamicPropNames && dynamicPropNames.length) {
                    vnodeDynamicProps = stringifyDynamicPropNames(
                        dynamicPropNames
                    );
                }
            }
    ...
    const transformOn = (dir, node, context, augmentor) => {
        ...
        if (!dir.exp && !modifiers.length) {
            context.onError(
                createCompilerError(34 /* X_V_ON_NO_EXPRESSION */, loc)
            );
        }
        let eventName;
    ...
    const transformModel = (dir, node, context) => {
        ...
        if (!isMemberExpression(expString)) {
            context.onError(
                createCompilerError(
                    41 /* X_V_MODEL_MALFORMED_EXPRESSION */,
                    exp.loc
                )
            );
            return createTransformProps();
    ...
    function baseCompile(template, options = {}) {
        ...
        /* istanbul ignore if */
        {
            if (options.prefixIdentifiers === true) {
                onError(
                    createCompilerError(45 /* X_PREFIX_ID_NOT_SUPPORTED */)
                );
            } else if (isModuleMode) {
                onError(
                    createCompilerError(46 /* X_MODULE_MODE_NOT_SUPPORTED */)
                );
            }
        }
        ...
        if (options.cacheHandlers) {
            onError(
                createCompilerError(47 /* X_CACHE_HANDLER_NOT_SUPPORTED */)
            );
        }
    ...
    const transformModel$1 = (dir, node, context) => {
        ...
        if (
            ...
            context.onError(
                createDOMCompilerError(
                    53 /* X_V_MODEL_ON_INVALID_ELEMENT */,
                    dir.loc
                )
            );
        }
            ...
            if (component === TRANSITION$1) {
                return () => {
                    if (node.children.length && hasMultipleChildren(node)) {
                        context.onError(
                            createDOMCompilerError(
                                58 /* X_TRANSITION_INVALID_CHILDREN */,
                                {
                                    start: node.children[0].loc.start,
                                    end:
                                        node.children[node.children.length - 1]
